...
class LAPPDHit : public G4VHit
{
  public:

      LAPPDHit();
      LAPPDHit(G4Step* aStep);
      ~LAPPDHit();
      //LAPPDHit(const LAPPDHit &right);
      //const LAPPDHit& operator=(const LAPPDHit &right); //same as implicit definition
      int operator==(const LAPPDHit &right) const;
...
typedef G4THitsCollection<LAPPDHit> LAPPDHitsCollection;

extern G4Allocator<LAPPDHit> LAPPDHitAllocator;

inline void* LAPPDHit::operator new(size_t)
{
  return (void *) LAPPDHitAllocator.MallocSingle();
}

inline void LAPPDHit::operator delete(void *aHit)
{
  LAPPDHitAllocator.FreeSingle((LAPPDHit*) aHit);
}

inline G4int LAPPDHit::operator==
       (const LAPPDHit& right) const 
{
   return (this==&right) ? 1 : 0; 
}

/*inline const SBsimLAPPDHit& SBsimLAPPDHit::operator= (const SBsimMRDHit& right) {
