...
/***********************************************************
 *
 * This file contains the LAPPD QE as a function of 
 * wavelength for variety of LAPPDs.  It is used in both the
...
G4float WCSimDetectorConstruction::GetLAPPDQE(G4String CollectionName, G4float PhotonWavelength, G4int flag, G4float low_wl, G4float high_wl, G4float ratio){
  // XQ  08/17/10
  ...
  LAPPD = GetLAPPDPointer(CollectionName);
  G4float *wavelength;
  wavelength = LAPPD->GetQEWavelength();
  G4float *QE;
  QE = LAPPD->GetQE();
  G4float maxQE;
  maxQE = LAPPD->GetmaxQE();
